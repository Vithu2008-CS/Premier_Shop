...
html[data-admin-theme="dark"] .stats-details-row {
...
<div class="container-fluid px-0" style="padding-bottom: 120px;">
    ...
    <div class="row">
        ...
        <div class="col-lg-7 mb-4">
            <form action="{{ route('admin.coupons.update', $coupon) }}" method="POST" id="coupon-edit-form" class="d-flex flex-column gap-4 w-100">
                @csrf
                @method('PUT')
                        ...
                        <div class="row">
                            <div class="col-md-4 mb-3">
                                ...
                                <input type="number" name="min_order_amount" id="min_order" class="form-control"
                                       step="0.01" min="0" value="{{ old('min_order_amount', $coupon->min_order_amount) }}" placeholder="0.00">
                            </div>
                            <div class="col-md-4 mb-3">
            ...
            <div class="d-flex flex-column gap-4 h-100 justify-content-between">
                ...
                <div class="card coupon-ticket-preview p-4 d-flex flex-column justify-content-between position-relative" style="min-height: 240px;">
                    ...
                    <div class="d-flex justify-content-between align-items-center">
                        <div>
                            ...
                            <span class="font-weight-bold" id="preview_code_display" style="font-family: monospace; font-size: 1.15rem; letter-spacing: 1px;">{{ $coupon->code }}</span>
                        </div>
                ...
                <div class="card border-0 shadow-sm theme-card-bg p-4 rounded-4 mt-2">
                    <h6 class="card-title fw-bold text-primary mb-3 d-flex align-items-center" style="font-family:'Outfit',sans-serif; font-size: 0.9rem;">
                        <i class="bi bi-bar-chart mr-2" style="margin-right: 6px;"></i> Coupon Usage Stats
                    </h6>
                    
                    <div class="d-flex align-items-center justify-content-between p-2.5 rounded-3 mb-2 small text-muted stats-details-row">
                        <span>Times Used</span>
                        <span class="font-weight-bold text-theme-dark-bold" style="font-size: 0.82rem;">{{ $coupon->times_used }} times</span>
                    </div>
                    @if($coupon->usage_limit)
                    <div class="d-flex align-items-center justify-content-between p-2.5 rounded-3 small text-muted stats-details-row">
                        <span>Remaining Uses</span>
                        <span class="font-weight-bold text-theme-dark-bold" style="font-size: 0.82rem;">{{ max(0, $coupon->usage_limit - $coupon->times_used) }} uses</span>
                    </div>
                    @endif
                </div>
            </div>
...
<div class="floating-save-bar d-flex align-items-center justify-content-between px-4 py-3">
    <div class="d-flex align-items-center" style="font-family:'Outfit',sans-serif;gap:8px;">
        ...
        <span class="text-muted text-uppercase d-none d-sm-inline" style="font-size:0.68rem;letter-spacing:0.5px;font-weight:600;white-space:nowrap;">Active Coupon:</span>
        <span class="font-weight-bold text-nowrap floating-bar-title" style="font-size:0.85rem; font-family: monospace; letter-spacing: 0.5px;">{{ $coupon->code }}</span>
    </div>
    <div class="button-group">
        ...
        <button type="submit" form="coupon-edit-form" class="btn btn-primary">
            <i class="bi bi-check2-circle" style="margin-right:6px;"></i> Update Coupon
        </button>
    </div>
</div>

{{-- Hidden delete form --}}
<form id="delete-coupon-form" action="{{ route('admin.coupons.destroy', $coupon) }}" method="POST" style="display:none;">
    @csrf
    @method('DELETE')
</form>
@endsection

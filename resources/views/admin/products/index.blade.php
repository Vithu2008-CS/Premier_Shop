                    ...
                    <tr>
                        <td>
                            <div class="d-flex align-items-center gap-3">
                                <div style="width:44px;height:44px;border-radius:10px;overflow:hidden;background:rgba(255,255,255,0.05);flex-shrink:0;">
                        ...
                        <td>
                            @if($product->stock < 10)
                                <span class="badge badge-status bg-danger">{{ $product->stock }} <i class="bi bi-exclamation-triangle-fill ms-1"></i></span>
                        ...
                        <td>
                            <span class="badge badge-status" style="background:rgba({{ $product->product_type === 'wholesale' ? '0,206,201' : '255,255,255' }},0.1);color:{{ $product->product_type === 'wholesale' ? '#00CEC9' : 'var(--admin-muted)' }};">
                        ...
                        <td>
                            <div class="d-flex gap-1">
                                <a href="{{ route('admin.products.edit', $product) }}" class="btn-icon" title="Edit"><i class="bi bi-pencil"></i></a>
                                @if($product->qr_code)
                                    <a href="{{ $product->qr_code }}" download class="btn-icon" title="Download QR"><i class="bi bi-qr-code"></i></a>
                                @endif
                                <form action="{{ route('admin.products.destroy', $product) }}" method="POST" class="d-inline" onsubmit="return confirm('Delete this product?')">
                                    @csrf @method('DELETE')
                                    <button type="submit" class="btn-icon btn-icon-danger" title="Delete"><i class="bi bi-trash3"></i></button>
                                </form>
                            </div>
                        </td>
                    </tr>
                @empty
                    <tr><td colspan="7" class="text-center py-5" style="color:var(--admin-muted);">
                        <i class="bi bi-box-seam" style="font-size:2.5rem;display:block;margin-bottom:10px;"></i>
                        No products yet. <a href="{{ route('admin.products.create') }}" style="color:#A29BFE;">Add your first product</a>
                    </td></tr>
                @endforelse
            </tbody>
        </table>
